...
df_ipca.rename(columns={'data': 'Date', 'valor': 'Inflation Value'}, inplace=True)
df_ipca['Date'] = pd.to_datetime(df_ipca['Date'], format='%d/%m/%Y')
df_ipca['Inflation Value'] = pd.to_numeric(df_ipca['Inflation Value']) / 100
...
for i in df_ipca.index:
    ...
    df_date['Inflation Value'] = ((1 + df_ipca['Inflation Value'].values[i])**(1/df_ipca['Number of Days'].values[i])) - 1
